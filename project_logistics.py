...
THRESHOLD = 30 # hyperparameter for top words below 'code', 30 is the best so far
...
def main():
    ...
    preprocess = ColumnTransformer(
        transformers=[
            ...
            ("scale_nums", StandardScaler(), NUMERIC_COL) # normalize numeric features
        ],
    ...
    # extract label
    t_train = np.array(df_train["label"])
    t_val = np.array(df_val["label"])
    t_test = np.array(df_test["label"])
